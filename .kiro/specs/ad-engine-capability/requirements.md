# 需求文档 - Ad Engine Capability（广告引擎能力模块）

## 简介（Introduction）

Ad Engine Capability 是 Unified AI Agent 的能力模块之一，负责广告创建、管理和优化相关的业务逻辑。该模块被 Unified AI Agent 调用，通过 MCP 协议与 User Portal 通信进行数据存储，专注于自动化广告投放和预算优化。

## 术语表（Glossary）

- **Ad Engine Capability**：广告引擎能力模块
- **Campaign**：广告系列
- **Adset**：广告组
- **Ad**：广告
...
Ad Engine Capability 的所有接口协议详见：**[INTERFACES.md](../INTERFACES.md)**
...
- ❌ 报表分析（由 Reporting Capability 负责）
...
详见：[INTERFACES.md - Ad Engine Capability](../INTERFACES.md#ad-engine-capability)
...
**用户故事**：作为 Unified AI Agent，我需要调用 Ad Engine Capability 创建广告，以便自动生成最优的广告结构。

#### 验收标准

1. WHEN 调用 create_campaign action THEN Ad Engine Capability SHALL 自动创建 Campaign
2. WHEN Campaign 创建 THEN Ad Engine Capability SHALL 根据目标国家自动创建多个 Adset
3. WHEN Adset 创建 THEN Ad Engine Capability SHALL 自动配置受众定位（Broad Audience 或 Lookalike）
4. WHEN Adset 创建 THEN Ad Engine Capability SHALL 自动配置版位（自动版位）
5. WHEN Adset 创建 THEN Ad Engine Capability SHALL 自动配置出价策略（最低成本）

---

### 需求 2：素材自动挂载

**用户故事**：作为 Unified AI Agent，我需要将素材挂载到广告，以便快速发布。

#### 验收标准

1. WHEN 提供 creative_ids THEN Ad Engine Capability SHALL 从 User Portal 获取素材信息
2. WHEN 素材获取 THEN Ad Engine Capability SHALL 自动创建 Ad 并挂载素材
3. WHEN 素材有多个 THEN Ad Engine Capability SHALL 为每个素材创建独立的 Ad
4. WHEN 素材挂载 THEN Ad Engine Capability SHALL 自动生成广告文案（使用 AI）
5. WHEN 文案生成 THEN Ad Engine Capability SHALL 返回文案供确认

---

### 需求 3：自动预算优化

**用户故事**：作为 Unified AI Agent，我需要优化广告预算，以便提升广告效果。

#### 验收标准

1. WHEN 调用 optimize_budget action THEN Ad Engine Capability SHALL 分析广告表现数据
2. WHEN CPA 超过目标 150% THEN Ad Engine Capability SHALL 自动降低预算 20%
3. WHEN ROAS 超过目标 150% THEN Ad Engine Capability SHALL 自动提升预算 20%
4. WHEN 无转化 3 天 THEN Ad Engine Capability SHALL 自动暂停 Adset
5. WHEN 执行优化 THEN Ad Engine Capability SHALL 返回优化操作列表和原因

---

### 需求 4：广告系列管理

**用户故事**：作为 Unified AI Agent，我需要管理广告系列，以便执行用户的操作请求。

#### 验收标准

1. WHEN 调用 manage_campaign action THEN Ad Engine Capability SHALL 执行指定操作（暂停/启动/删除）
2. WHEN 操作执行 THEN Ad Engine Capability SHALL 调用广告平台 API
3. WHEN API 调用成功 THEN Ad Engine Capability SHALL 通过 MCP 更新 User Portal 数据
4. WHEN API 调用失败 THEN Ad Engine Capability SHALL 自动重试最多 3 次
5. WHEN 操作完成 THEN Ad Engine Capability SHALL 返回新状态和确认消息
...
**用户故事**：作为 Unified AI Agent，我需要创建自动化规则，以便系统自动执行操作。

#### 验收标准

1. WHEN 调用 create_rule action THEN Ad Engine Capability SHALL 创建自动化规则
2. WHEN 规则创建 THEN Ad Engine Capability SHALL 每 6 小时检查一次条件
3. WHEN 条件满足 THEN Ad Engine Capability SHALL 执行对应操作
4. WHEN 操作执行 THEN Ad Engine Capability SHALL 记录执行日志
5. WHEN 规则触发 THEN Ad Engine Capability SHALL 返回触发通知

---

### 需求 7：多平台支持

**用户故事**：作为 Unified AI Agent，我需要支持多个广告平台，以便统一管理。

#### 验收标准

1. WHEN 指定 platform 为 "meta" THEN Ad Engine Capability SHALL 调用 Meta Marketing API
2. WHEN 指定 platform 为 "tiktok" THEN Ad Engine Capability SHALL 调用 TikTok Ads API
3. WHEN 指定 platform 为 "google" THEN Ad Engine Capability SHALL 调用 Google Ads API
4. WHEN 跨平台操作 THEN Ad Engine Capability SHALL 确保操作一致性
5. WHEN 平台 API 失败 THEN Ad Engine Capability SHALL 返回平台特定的错误信息

---

### 需求 8：广告状态查询

**用户故事**：作为 Unified AI Agent，我需要查询广告状态，以便向用户报告表现。

#### 验收标准

1. WHEN 调用 get_campaign_status action THEN Ad Engine Capability SHALL 从广告平台获取实时数据
2. WHEN 数据获取 THEN Ad Engine Capability SHALL 返回 Campaign、Adset、Ad 的完整状态
3. WHEN 返回数据 THEN Ad Engine Capability SHALL 包含花费、收入、ROAS、CPA 等关键指标
4. WHEN 数据不可用 THEN Ad Engine Capability SHALL 返回最近一次缓存的数据
5. WHEN 查询失败 THEN Ad Engine Capability SHALL 返回清晰的错误信息
...
1. WHEN 广告平台 API 调用失败 THEN Ad Engine Capability SHALL 自动重试最多 3 次
2. WHEN 网络超时 THEN Ad Engine Capability SHALL 设置 30 秒超时并重试
3. WHEN 达到 API 限额 THEN Ad Engine Capability SHALL 返回限额错误并建议稍后重试
4. WHEN 达到重试上限 THEN Ad Engine Capability SHALL 返回明确的错误信息
5. WHEN 发生错误 THEN Ad Engine Capability SHALL 记录详细的错误日志
...
1. Ad Engine Capability SHALL 在 10 秒内完成广告创建
2. Ad Engine Capability SHALL 在 5 秒内完成预算优化
3. Ad Engine Capability SHALL 在 3 秒内返回广告状态
4. Ad Engine Capability SHALL 支持 10 个并发操作

### 可靠性需求

1. Ad Engine Capability SHALL 在 API 调用失败时自动重试
2. Ad Engine Capability SHALL 记录所有操作日志供审计
3. Ad Engine Capability SHALL 在系统重启后自动恢复规则引擎
4. Ad Engine Capability SHALL 确保广告操作的原子性（全部成功或全部回滚）

### 安全需求

1. Ad Engine Capability SHALL 验证用户权限后才执行操作
2. Ad Engine Capability SHALL 限制单次预算调整幅度（最大 50%）
3. Ad Engine Capability SHALL 记录所有敏感操作（删除、大额预算调整）
4. Ad Engine Capability SHALL 加密存储广告平台 API 密钥

### 成本控制需求

1. Ad Engine Capability SHALL 控制单次广告创建成本不超过 $0.10
2. Ad Engine Capability SHALL 缓存广告状态数据（5 分钟）
3. Ad Engine Capability SHALL 监控 API 调用成本并报警
...
class AdEngineCapability:
    ...
    async def execute(self, action: str, parameters: dict, context: dict) -> dict:
        try:
            ...
            logger.error(f"Ad engine capability error: {e}")
