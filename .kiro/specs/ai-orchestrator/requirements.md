...
│                                                                  │
│  流程：                                                          │
│  1. generate_creative → 上传图片到 GCS（永久）                    │
│  2. 存储元数据到 Redis（追踪待保存状态）                          │
│  3. 用户预览图片（GCS URL 永久有效）                             │
│  4. 用户说"保存" → save_creative → MCP 写入 Backend 素材库       │
│  5. Redis 元数据过期后，图片仍可通过 GCS URL 在聊天历史中显示     │
└─────────────────────────────────────────────────────────────────┘
```
...
| 方案 | 问题 | 采用方案的优势 |
|------|------|---------------|
| Redis 存图片 | 30 分钟过期，聊天历史无法显示 | ❌ 用户体验差 |
| 直接写 Backend | 用户可能不满意，产生垃圾数据 | ❌ 浪费存储 |
| **GCS 永久 + Redis 追踪** | — | ✅ 图片永久可见，按需保存到素材库 |

**关键文件**：
- `app/services/gcs_client.py` - GCS 上传客户端
- `app/services/temp_storage.py` - Redis 元数据存储（仅存 URL，不存图片）
- `app/nodes/creative_node.py` - 素材生成节点
- `app/nodes/save_creative_node.py` - 素材保存节点

#### 6.2 保存素材意图（save_creative）

用户生成素材后，可以通过以下方式保存到素材库：

**触发词**：保存、保存素材、存到素材库、保存图片

**流程**：
1. 用户说"保存素材"
2. 意图识别为 `save_creative`
3. 从 Redis 获取待保存的元数据
4. 调用 MCP `create_creative` 写入 Backend
5. 清理 Redis 元数据
...
#### 11.1 MCP 与系统级 API 的边界划分

**设计决策**（2024-11-30 更新）：

MCP 协议仅用于将后端能力作为**工具（Tools）暴露给 AI Agent**，让 Agent 能够理解和调用。
系统级基础设施操作（如 Credit 管理）不应使用 MCP，而应通过**直接 HTTP API** 通信。
...
│  │                    MCP 协议                               │   │
│  │                                                           │   │
│  │  适用于：AI Agent 需要理解和决策的操作                      │   │
│  │                                                           │   │
│  │  ✅ create_creative - Agent 决定何时保存素材               │   │
│  │  ✅ create_campaign - Agent 决定创建什么广告               │   │
│  │  ✅ get_reports - Agent 决定获取什么数据                   │   │
│  │  ✅ analyze_competitor - Agent 决定分析什么竞品            │   │
│  │                                                           │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                   直接 HTTP API                           │   │
│  │                                                           │   │
│  │  适用于：系统级基础设施，对 AI 模型透明                    │   │
│  │                                                           │   │
│  │  ✅ Credit 检查/扣除/退款 - LLM 调用时自动触发             │   │
│  │  ✅ 用户认证验证 - 请求前自动验证                          │   │
│  │  ✅ 日志记录 - 系统自动记录                                │   │
│  │  ✅ 监控指标 - 系统自动采集                                │   │
│  │                                                           │   │
...
| 考虑因素 | MCP (Tool) | 直接 HTTP API |
|---------|-----------|---------------|
| AI 需要理解吗？ | 是 | ❌ 不需要 |
| AI 需要决策吗？ | 是 | ❌ 不需要 |
| 执行时机 | Agent 主动调用 | 系统自动触发 |
| 典型场景 | "保存这个素材" | LLM 消耗 token 后自动扣费 |

Credit 管理是**系统级基础设施**：
- AI 模型不需要"理解"什么是 Credit
- AI 模型不需要"决定"是否扣费
- Credit 扣除在 LLM 调用后自动发生

---

### 需求 11.2：Credit 管理（系统级 API）

**用户故事**：作为系统，我需要在 AI 操作时自动管理 Credit 余额。

#### 验收标准

1. WHEN 用户发起 AI 操作 THEN 系统 SHALL 先检查 Credit 余额
2. WHEN Credit 余额不足 THEN 系统 SHALL 返回 6011 错误码
3. WHEN AI 操作成功 THEN 系统 SHALL 扣除相应 Credit
4. WHEN AI 操作失败 THEN 系统 SHALL 退还已扣除的 Credit
5. WHEN 扣除失败 THEN 系统 SHALL 重试 3 次后返回 6012 错误码

#### Credit API 端点

```
POST /api/v1/credits/check   - 检查余额是否足够
POST /api/v1/credits/deduct  - 扣除 Credit
POST /api/v1/credits/refund  - 退还 Credit
GET  /api/v1/credits/balance - 获取余额
```

**关键文件**：
- `app/services/credit_client.py` - Credit API 客户端（直接 HTTP，非 MCP）
