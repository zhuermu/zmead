# 需求文档 - Market Intelligence Capability（市场洞察能力模块）

## 简介（Introduction）

Market Intelligence Capability 是 Unified AI Agent 的能力模块之一，负责市场分析和竞品洞察相关的业务逻辑。该模块被 Unified AI Agent 调用，通过 MCP 协议与 User Portal 通信进行数据存储，专注于为用户提供 AI 驱动的市场洞察和广告策略建议。

## 术语表（Glossary）

- **Market Intelligence Capability**：市场洞察能力模块
...
Market Intelligence Capability 的所有接口协议详见：**[INTERFACES.md](../INTERFACES.md)**
...
- ❌ 广告创建（由 Ad Engine Capability 负责）
...
```python
class MarketIntelligenceCapability:
    async def execute(
        self,
        action: str,
        parameters: dict,
...
详见：[INTERFACES.md - Market Intelligence Capability](../INTERFACES.md#market-intelligence-capability)
...
**用户故事**：作为 Unified AI Agent，我需要调用 Market Intelligence Capability 分析竞品，以便为用户提供竞争洞察。

#### 验收标准

1. WHEN 调用 analyze_competitor action THEN Market Intelligence Capability SHALL 分析竞品产品信息
2. WHEN 分析完成 THEN Market Intelligence Capability SHALL 返回竞品的卖点、定价策略、目标受众
3. WHEN 分析竞品 THEN Market Intelligence Capability SHALL 识别竞品的优势和劣势
4. WHEN 生成建议 THEN Market Intelligence Capability SHALL 提供差异化竞争建议
...
**用户故事**：作为 Unified AI Agent，我需要获取热门广告素材，以便帮助用户了解市场趋势。

#### 验收标准

1. WHEN 调用 get_trending_creatives action THEN Market Intelligence Capability SHALL 调用 TikTok Creative Center API
2. WHEN API 调用成功 THEN Market Intelligence Capability SHALL 返回热门素材列表
3. WHEN 返回素材 THEN Market Intelligence Capability SHALL 包含播放量、互动率、平台信息
4. WHEN 调用 analyze_creative_trend THEN Market Intelligence Capability SHALL 使用 AI Vision 分析素材特点
5. WHEN 分析完成 THEN Market Intelligence Capability SHALL 返回成功要素和优化建议

---

### 需求 3：市场趋势洞察

**用户故事**：作为 Unified AI Agent，我需要获取市场趋势数据，以便帮助用户把握商机。

#### 验收标准

1. WHEN 调用 get_market_trends action THEN Market Intelligence Capability SHALL 调用 Google Trends API
2. WHEN API 调用成功 THEN Market Intelligence Capability SHALL 返回搜索趋势数据
3. WHEN 返回趋势 THEN Market Intelligence Capability SHALL 包含搜索量、增长率、趋势方向
4. WHEN 分析趋势 THEN Market Intelligence Capability SHALL 识别热门话题和新兴趋势
5. WHEN 提供洞察 THEN Market Intelligence Capability SHALL 包含季节性模式和相关查询

---

### 需求 4：AI 广告策略生成

**用户故事**：作为 Unified AI Agent，我需要生成 AI 广告策略，以便为用户提供投放建议。

#### 验收标准

1. WHEN 调用 generate_ad_strategy action THEN Market Intelligence Capability SHALL 使用 AI 模型生成策略
2. WHEN 策略生成完成 THEN Market Intelligence Capability SHALL 返回受众推荐
3. WHEN 策略生成完成 THEN Market Intelligence Capability SHALL 返回素材方向建议
4. WHEN 策略生成完成 THEN Market Intelligence Capability SHALL 返回预算规划建议
5. WHEN 策略生成完成 THEN Market Intelligence Capability SHALL 返回广告系列结构建议

---

### 需求 5：策略效果追踪

**用户故事**：作为 Unified AI Agent，我需要追踪策略效果，以便评估 AI 建议的质量。

#### 验收标准

1. WHEN 调用 track_strategy_performance action THEN Market Intelligence Capability SHALL 对比采纳和未采纳策略的广告表现
2. WHEN 对比完成 THEN Market Intelligence Capability SHALL 计算 ROAS、CTR、转化率的提升百分比
3. WHEN 返回结果 THEN Market Intelligence Capability SHALL 包含详细的性能对比数据
4. WHEN 生成洞察 THEN Market Intelligence Capability SHALL 提供策略效果的文字说明
5. WHEN 追踪失败 THEN Market Intelligence Capability SHALL 返回具体的失败原因
...
1. WHEN 调用 TikTok Creative Center API THEN Market Intelligence Capability SHALL 使用官方 API 密钥
2. WHEN 调用 pytrends THEN Market Intelligence Capability SHALL 遵守速率限制（每分钟最多 5 次请求）
3. WHEN API 调用失败 THEN Market Intelligence Capability SHALL 自动重试最多 3 次，间隔指数退避
4. WHEN 达到 API 限额 THEN Market Intelligence Capability SHALL 使用缓存数据或降级方案
5. WHEN API 响应慢 THEN Market Intelligence Capability SHALL 设置 30 秒超时
6. WHEN 数据源完全不可用 THEN Market Intelligence Capability SHALL 返回明确的降级提示和可用的部分数据
...
1. WHEN 获取趋势数据 THEN Market Intelligence Capability SHALL 缓存结果 24 小时
2. WHEN 获取热门素材 THEN Market Intelligence Capability SHALL 缓存结果 12 小时
3. WHEN 缓存命中 THEN Market Intelligence Capability SHALL 直接返回缓存数据
4. WHEN 缓存过期 THEN Market Intelligence Capability SHALL 重新调用 API 并更新缓存
5. WHEN 缓存失败 THEN Market Intelligence Capability SHALL 降级到直接 API 调用
...
1. Market Intelligence Capability SHALL 在 15 秒内完成竞品分析
2. Market Intelligence Capability SHALL 在 10 秒内返回热门素材列表
3. Market Intelligence Capability SHALL 在 20 秒内生成完整的广告策略
4. Market Intelligence Capability SHALL 支持 5 个并发分析任务

### 数据质量需求

1. Market Intelligence Capability SHALL 仅使用公开 API 获取数据（避免爬虫）
2. Market Intelligence Capability SHALL 每周自动更新趋势数据
3. Market Intelligence Capability SHALL 仅分析公开可访问的信息
4. Market Intelligence Capability SHALL 确保分析结果的准确性 > 80%

### 合规性需求

1. Market Intelligence Capability SHALL 不存储竞品的原始素材
2. Market Intelligence Capability SHALL 仅存储分析结果
3. Market Intelligence Capability SHALL 遵守各平台的 API 使用条款
4. Market Intelligence Capability SHALL 遵守数据隐私法规（GDPR、CCPA）

### 成本控制需求

1. Market Intelligence Capability SHALL 控制单次分析成本不超过 $0.20
2. Market Intelligence Capability SHALL 使用缓存减少 API 调用次数
3. Market Intelligence Capability SHALL 监控 API 调用成本并报警
...
class MarketIntelligenceCapability:
    ...
    async def execute(self, action: str, parameters: dict, context: dict) -> dict:
        try:
            ...
            logger.error(f"Market intelligence capability error: {e}")
